...
```bash
ls ~/.codex/hud/omc-hud.mjs 2>/dev/null && echo "EXISTS" || echo "MISSING"
```

**Step 2:** Check if the plugin is built (CRITICAL - common issue!):
```bash
# Find the latest version and check if dist/hud/index.js exists
PLUGIN_VERSION=$(ls ~/.codex/plugins/cache/omc/oh-my-codex/ 2>/dev/null | sort -V | tail -1)
if [ -n "$PLUGIN_VERSION" ]; then
  ls ~/.codex/plugins/cache/omc/oh-my-codex/$PLUGIN_VERSION/dist/hud/index.js 2>/dev/null && echo "BUILT" || echo "NOT_BUILT"
fi
```
...
```bash
cd ~/.codex/plugins/cache/omc/oh-my-codex/$PLUGIN_VERSION && npm install
```
...
async function main() {
  ...
  const pluginCacheBase = join(home, ".codex/plugins/cache/omc/oh-my-codex");
  ...
  if (pluginCacheDir) {
    ...
    console.log("[OMC] Plugin not found. Run: $omc-setup");
  }
}
...
```bash
node -e "const p=require('path').join(require('os').homedir(),'.codex','hud','omc-hud.mjs');console.log(JSON.stringify(p))"
```
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node /home/username/.codex/hud/omc-hud.mjs"
  }
}
```
...
HUD config is stored at: `~/.codex/.omc/hud-config.json`
...
- Settings: `~/.codex/settings.json` should have `statusLine` configured
