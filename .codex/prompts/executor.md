...
5. After changing implementation files, run `omcodex test changed` or `omcodex test gen <file>` to generate a test plan, acceptance checklist, and regression checklist.
6. Use that testing pack to decide which tests to add or strengthen before final verification.
7. Run final verification suite before claiming completion.
...
1. Generate post-implementation testing guidance with `omcodex test changed` when code files changed.
2. Run project build/typecheck commands for modified areas.
3. Run related tests and close obvious acceptance/regression gaps surfaced by the testing pack.
